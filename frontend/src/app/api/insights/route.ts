...
export async function GET(req: NextRequest) {
  ...
  const summary = await queryOne(`
    SELECT 
      COUNT(DISTINCT a.plant_id)::int AS plants_with_attribution,
      COUNT(DISTINCT a.plant_id) FILTER (WHERE a.pct_change_total < 0)::int AS plants_improving,
      COUNT(DISTINCT a.plant_id) FILTER (WHERE a.pct_change_total > 0)::int AS plants_worsening,
      ROUND(AVG(a.pct_change_total)::numeric,1)::float AS avg_gwp_change_pct
