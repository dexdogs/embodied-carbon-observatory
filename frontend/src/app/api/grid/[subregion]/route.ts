...
export async function GET(_: NextRequest, { params }: { params: { subregion: string } }) {
  ...
  `, [params.subregion.toUpperCase()])
  if (!records.length) return NextResponse.json({ detail: 'Not found' }, { status: 404 })
  return NextResponse.json({ subregion: params.subregion.toUpperCase(), history: records })
}
